...
func ExecuteBallerinaCommand(javaCmd string, cmdLineArgs []string) {
	// Construct the command with Java command and command-line arguments
	cmdArgs := append(cmdLineArgs, "io.ballerina.cli.launcher.Main")
	cmdArgs = append(cmdArgs, os.Args[1:]...)
	_ = ExecuteCommand(javaCmd, cmdArgs)
